...
export class InvoiceService {
    ...
    try {
      ...
      if (invoiceData.invoiceType === 'purchase') {
        ...
        for (const entry of entries) {
          ...
          group.totalAmount += exactWeight * rate;
          group.entries.push(entry);
        }

        // Convert to array and calculate totals
        materialBreakdown = Array.from(materialGroups.values());
        totalQuantity = materialBreakdown.reduce((sum, item) => sum + item.totalQuantity, 0);
  ...
  private static async generatePdfContent(doc: PDFKit.PDFDocument, invoice: any): Promise<void> {
    ...
    doc.font('Helvetica');
    doc.text(
      `${invoice.startDate.toLocaleDateString()} - ${invoice.endDate.toLocaleDateString()}`,
      margin + 50,
      currentY,
    );
    ...
    // Footer (only if there's space)
    if (currentY < pageHeight - 60) {
      doc.fontSize(6).font('Helvetica');
      doc.fillColor('#7f8c8d');
      doc.text(
        'Generated by Biofuel Management System',
        margin + contentWidth / 2,
        pageHeight - 40,
        { align: 'center' },
      );
